...
% to create a separate script and write a subscriber to the "/keypress"
% topic to read in the keyboard character, then write three publishers for
% each of the LED topics ("/red_led", "/green_led", "/blue_led"). Start out
% with turning the LEDs on and off whenever the user presses the 'r', 'g'
% and 'b' keys. If the red LED is currently off when the 'r' key is
% pressed, turn the LED on. If it is currently on when the LED is pressed,
% turn the LED off. When you have successfully implemented this feature
% have some fun with it and make your own unique pattern with any key
% sequence.
%
...
while (true)
    ...
    % (uncomment if you want the plot to stay in the foreground at all
    % times)
    %figure(1);
end
